...
class Auth
{
    ...
    public function __construct($AccountName = "", $password ="" , $username = "" , $Phone = "", $address = "" ,$userEmail = "" ,$role=0)
       {
    ...
    public static function login($pdo,$TenTK,$password)
    {
        $sql = "SELECT TenTK,MK,SDT,DiaChi,Role FROM taikhoan WHERE TenTK = :TenTK ";
        ...
        if ($stmt==true) {
                if($TenTK =='admin')   {
                    $_SESSION['login_admin'] =$TenTK;
                    $_SESSION['admin'] = $TenTK;
                    header('location: admin/index.php');
                }
                else{
                    $_SESSION['login_detail'] =$TenTK;
                    $_SESSION['name'] = $TenTK;
                    $_SESSION['id'] =$user['TenTK'];
